...
const actions = {
    requestFirstEntrySuggestions({ commit }, voc) {
        return new Promise((resolve, reject) => {
            const req = `${import.meta.env.api.baseUrl}search?filter=vocabulary&vocabulary=${voc}&autocomplete=true`;
            axios.get(req)
            .then((res) => {
                resolve(res);
            })
    ...
    requestAutocompleteSuggestions({ commit }, { voc, text }) {
        return new Promise((resolve, reject) => {
            const input = text;
            const req = `${import.meta.env.api.baseUrl}search?filter=vocabulary&vocabulary=${voc}&autocomplete=true&q=${input}`;
            axios.get(req)
            .then((res) => {
                resolve(res);
            })
    ...
    requestResourceName({ commit }, { voc, resource }) {
        ...
        if (voc === 'iana-media-types' || voc === 'spdx-checksum-algorithm') {
            req = `${import.meta.env.api.baseUrl}vocabularies/${voc}`;
           
        } else {
            req = `${import.meta.env.api.baseUrl}vocabularies/${voc}/${value}`;
