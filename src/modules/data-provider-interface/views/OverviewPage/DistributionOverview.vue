        ...
        <div class="mt-2">
            ...
            <div class="col-12">
                ...
                <ul class="list list-unstyled col-12" v-if="getData('distributions').length > 0">
                    <li class="row border-bottom" v-for="(distribution, id) in getData('distributions')" :key="`distribution${id+1}`">
                    ...
                    <span class="col-10 mb-3">
                        ...
                        <span class="d-inline-block col-12">
                            ...
                            <h6 class="font-weight-bold m-0">
                                {{ distribution['dct:title'].filter(el => el['@language'] === dpiLocale).map(el => el['@value'])[0] }}
                            </h6>
                            </span>
                            <!-- DISTRIBUTIONS DESCRIPTION -->
                            <span class="mt-2 text-muted text-truncate" v-if="showValue(distribution, 'dct:description')">
                            <small>
                                {{ distribution['dct:description'].filter(el => el['@language'] === dpiLocale).map(el => el['@value'])[0] }}
                            </small>
                            </span>
                        </span>
                        </span>
                        ...
                        <span class="d-inline-block col-12">
                            ...
                            <div v-for="(elem, index) in distribution['dcat:accessURL']" :key="index">
                        ...
                        <span class="d-inline-block col-12 mt-2">
                            <div v-for="(value, name, index) in tableProperties" :key="index">
                                <PropertyEntry profile="distributions" :data="getData('distributions')[id]" :property="name" :value="value" :dpiLocale="dpiLocale" :distId="id"></PropertyEntry>
                            </div>
                        </span>
                        </span>
                    </span>
